 ...
 * Parse build output to extract article counts
 */
function parseBuildCounts(output: string): {
  articleCount: number;
  categoryCounts: Record<string, number>;
} {
 ...
 * Run deploy command
 */
export function deployProject(options: DeployOptions = {}): {
  success: boolean;
  url: string;
  deploymentId?: string;
} {
 ...
 * Git add file
 */
export function gitAdd(filePath: string, projectPath: string): void {
 ...
 */
export function gitCommit(message: string, projectPath: string): string {
